class Service::Appaloosa < Service::Base
  ...
  string :url, :placeholder => 'Incoming Webhook URL',
         :label => 'Your Appaloosa Incoming Webhook URL. <br />' \
                   'You can find your incoming webhook url under issues section in application details.'
  page "URL", [ :url ]
  ...
  def receive_issue_impact_change(config, payload)
    ...
    if successful_response?(response)
      ...
      raise "Appaloosa WebHook issue create failed: HTTP status code: #{response.status}, body: #{response.body}"
    end
  end
